...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
  ...
  protected void configure(HttpSecurity http) throws Exception {
    http.csrf()
        .disable()
        .authorizeRequests()
        // Health Check APIs (Middleware) - Require Authentication (provided by fixed
        // token)
        .antMatchers("/api/hrm/health-check/**").authenticated()
        // Third-party employee APIs with JWT token - publicly accessible (token checked
        // in controller)
